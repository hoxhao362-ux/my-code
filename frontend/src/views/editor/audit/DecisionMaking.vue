...
// Filter for pending final decision journals (only accessible by editors and associate_editors)
const pendingFinalDecisionJournals = computed(() => {
  if (!user.value || !['editor', 'admin', 'associate_editor'].includes(user.value.role)) return []
  return userStore.journals.filter(journal => {

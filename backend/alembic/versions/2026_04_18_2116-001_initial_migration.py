...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('uid_hash', sa.Text(), nullable=False, comment='用户UID哈希（业务唯一）'),
        ...
        sa.Column('login_days', sa.Integer(), nullable=False, server_default='0', comment='累计登录天数'),
    ...
    op.create_table(
        'manuscripts',
        sa.Column('manuscript_id', sa.BigInteger(), nullable=False, comment='稿件 ID（自增，BIGSERIAL）'),
        sa.Column('author_uid', sa.Integer(), nullable=False, comment='投稿作者用户 ID'),
        sa.Column('title', sa.Text(), nullable=False, comment='稿件标题'),
        sa.Column('article_type', sa.Text(), nullable=False, server_default='Research Article', 
                  comment='文章类型'),
        sa.Column('section_category', sa.Text(), nullable=True, comment='栏目/类别'),
        sa.Column('keywords', sa.Text(), nullable=False, server_default='', comment='关键字'),
        sa.Column('first_author', sa.Text(), nullable=False, server_default='', comment='第一作者'),
        sa.Column('corresponding_author', sa.Text(), nullable=False, server_default='', comment='通讯作者'),
        sa.Column('order_of_authors', sa.Text(), nullable=False, server_default='[]', 
                  comment='作者排序(JSON格式)'),
        sa.Column('authors', sa.Text(), nullable=False, comment='作者列表（字符串，逗号分隔，向下兼容）'),
        sa.Column('abstract', sa.Text(), nullable=True, comment='摘要'),
        sa.Column('subject', sa.Text(), nullable=False, comment='学科/主题'),
        sa.Column('stage', sa.Text(), nullable=False, server_default='initial_review', 
                  comment='流转阶段：initial_review/peer_review/final_decision'),
        sa.Column('status', sa.Text(), nullable=False, server_default='pending_initial_review', 
                  comment='全局状态'),
        ...
        sa.ForeignKeyConstraint(['author_uid'], ['users.uid'], name='fk_manuscripts_author_uid_users')
    )
    ...
    op.create_table(
        'manuscript_versions',
        sa.Column('version_id', sa.BigInteger(), nullable=False, comment='版本 ID（自增）'),
        sa.Column('manuscript_id', sa.BigInteger(), nullable=False, comment='稿件 ID'),
        sa.Column('version_number', sa.Integer(), nullable=False, comment='版本号（从 1 开始）'),
        sa.Column('title', sa.Text(), nullable=False, comment='稿件标题'),
        ...
        sa.Column('file_name', sa.Text(), nullable=False, comment='原始文件名'),
        sa.Column('file_size', sa.BigInteger(), nullable=False, comment='文件大小（字节）'),
        sa.Column('submitted_by_uid', sa.Integer(), nullable=False, comment='提交者用户 ID'),
        ...
        sa.ForeignKeyConstraint(['manuscript_id'], ['manuscripts.manuscript_id'], 
                               name='fk_manuscript_versions_manuscript_id_manuscripts'),
        sa.ForeignKeyConstraint(['submitted_by_uid'], ['users.uid'], 
                               name='fk_manuscript_versions_submitted_by_uid_users')
    )
    op.create_index('idx_manuscript_versions_manuscript_id', 'manuscript_versions', ['manuscript_id'])
    op.create_index('idx_manuscript_versions_version_number', 'manuscript_versions', ['version_number'])
    ...
    op.create_table(
        ...
        sa.Column('participant_id', sa.Integer(), nullable=False, comment='参与者 ID（自增）'),
        sa.Column('manuscript_id', sa.BigInteger(), nullable=False, comment='稿件 ID'),
        ...
        sa.Column('assigned_by_uid', sa.Integer(), nullable=False, comment='分配者用户 ID'),
        ...
        sa.ForeignKeyConstraint(['assigned_by_uid'], ['users.uid'], 
                               name='fk_manuscript_participants_assigned_by_uid_users')
    )
    op.create_index('idx_manuscript_participants_manuscript_id', 'manuscript_participants', ['manuscript_id'])
    op.create_index('idx_manuscript_participants_user_uid', 'manuscript_participants', ['user_uid'])
    op.create_index('idx_manuscript_participants_role_type', 'manuscript_participants', ['role_type'])
    ...
    op.create_table(
        ...
        sa.Column('file_type', sa.Text(), nullable=False, server_default='attachment', 
                  comment='文件类型：main/review/letter/other'),
        sa.Column('uploaded_by_uid', sa.Integer(), nullable=False, comment='上传者用户 ID'),
        ...
        sa.ForeignKeyConstraint(['uploaded_by_uid'], ['users.uid'], 
                               name='fk_manuscript_files_uploaded_by_uid_users')
    )
    op.create_index('idx_manuscript_files_manuscript_id', 'manuscript_files', ['manuscript_id'])
    ...
    op.create_table(
        ...
        sa.Column('issue_number', sa.Text(), nullable=False, comment='期号（业务字段）'),
        sa.Column('publication_date', sa.Text(), nullable=False, comment='出版日期（ISO字符串）'),
    ...
    op.create_index('idx_manuscript_info_manuscript_id', 'manuscript_info', ['manuscript_id'])
    ...
    op.create_table(
        ...
        sa.Column('max_uses', sa.Integer(), nullable=False, server_default='1', comment='最大可使用次数'),
        ...
        sa.Column('created_by_uid', sa.Integer(), nullable=False, comment='创建人用户ID'),
        sa.Column('create_time', sa.DateTime(), nullable=False, comment='创建时间'),
    ...
    op.create_table(
        ...
        sa.Column('usage_id', sa.Integer(), nullable=False, comment='使用记录ID（自增）'),
        sa.Column('code', sa.Text(), nullable=False, comment='邀请码'),
        sa.Column('used_by_uid', sa.Integer(), nullable=False, comment='使用人用户ID'),
        sa.Column('used_by_username', sa.Text(), nullable=False, comment='使用人用户名'),
    ...
    op.create_index('idx_invitation_code_usage_used_by_uid', 'invitation_code_usage', ['used_by_uid'])
    ...
    op.create_table(
        ...
        sa.Column('review_round', sa.Integer(), nullable=False, server_default='1', comment='审稿轮次'),
        ...
        sa.Column('decision', sa.Text(), nullable=True, comment='审稿结论：accept/reject/revision'),
        sa.Column('submitted_at', sa.DateTime(), nullable=False, comment='提交时间'),
    ...
    op.create_index('idx_review_opinions_manuscript_id', 'review_opinions', ['manuscript_id'])
    op.create_index('idx_review_opinions_reviewer_uid', 'review_opinions', ['reviewer_uid'])
    ...
    op.create_table(
        ...
        sa.Column('user_uid', sa.Integer(), nullable=False, comment='用户 ID'),
        sa.Column('position', sa.Text(), nullable=False, comment='职位：editor/associate_editor/ea_ae'),
        sa.Column('title', sa.Text(), nullable=True, comment='职称/头衔'),
        sa.Column('research_areas', sa.Text(), nullable=True, comment='研究领域（逗号分隔）'),
        sa.Column('appointed_at', sa.DateTime(), nullable=False, comment='任命时间'),
        sa.Column('appointed_by_uid', sa.Integer(), nullable=False, comment='任命者用户 ID'),
        sa.Column('is_active', sa.Boolean(), nullable=False, server_default='true', comment='是否在职'),
        sa.PrimaryKeyConstraint('board_id', name='pk_editorial_board'),
        sa.UniqueConstraint('user_uid', name='uq_editorial_board_user_uid'),
        sa.ForeignKeyConstraint(['user_uid'], ['users.uid'], 
                               name='fk_editorial_board_user_uid_users'),
        sa.ForeignKeyConstraint(['appointed_by_uid'], ['users.uid'], 
    ...
    op.create_table(
        'decision_records',
        sa.Column('decision_id', sa.BigInteger(), nullable=False, comment='决策 ID（自增）'),
        sa.Column('manuscript_id', sa.BigInteger(), nullable=False, comment='稿件 ID'),
        sa.Column('stage', sa.Text(), nullable=False, 
        ...
        sa.Column('recommendations', sa.Text(), nullable=True, comment='建议'),
        sa.Column('decided_by_uid', sa.Integer(), nullable=False, comment='决策者用户 ID'),
        ...
        sa.ForeignKeyConstraint(['decided_by_uid'], ['users.uid'], 
                               name='fk_decision_records_decided_by_uid_users')
    )
    op.create_index('idx_decision_records_manuscript_id', 'decision_records', ['manuscript_id'])
    op.create_index('idx_decision_records_stage', 'decision_records', ['stage'])
    op.create_index('idx_decision_records_decided_by_uid', 'decision_records', ['decided_by_uid'])
    ...
    op.create_table(
        ...
        sa.Column('merchant_order_id', sa.Text(), nullable=False, comment='商户订单号（唯一）'),
        ...
        sa.Column('currency', sa.Text(), nullable=False, server_default='CNY', comment='币种'),
        sa.Column('subject', sa.Text(), nullable=False, comment='订单标题/商品描述'),
        sa.Column('provider', sa.Text(), nullable=False, comment='支付提供商（alipay/wechat/paypal/mock...）'),
        ...
        sa.Column('extra_data', sa.Text(), nullable=True, comment='扩展数据（JSON字符串等）'),
        ...
        sa.ForeignKeyConstraint(['uid'], ['users.uid'], name='fk_payment_orders_uid_users')
    )
    op.create_index('idx_payment_orders_merchant_order_id', 'payment_orders', ['merchant_order_id'])

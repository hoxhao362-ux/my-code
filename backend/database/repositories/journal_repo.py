...
class JournalRepository(BaseRepository[Journal]):
    ...
    async def list_by_uploader_page(
        self, uid: int, page: int, page_size: int
    ) -> list[Dict[str, Any]]:
        """按上传者分页查询文献列表"""
    ...
    async def list_all_with_uploader_page(
        ...
        rows = (
            (
                await self.session.execute(
                    stmt.order_by(Journal.create_time.desc())
                    .limit(page_size)
                    .offset(offset)
                )
            )
    ...
    async def list_deleted_with_uploader_page(
        self, page: int, page_size: int
    ) -> list[Dict[str, Any]]:
        """查询已删除文献（含上传者信息），分页返回"""
        offset = (page - 1) * page_size
    ...
    async def status_breakdown(self) -> list[Dict[str, Any]]:
        ...
        rows = (
            (
                await self.session.execute(
                    select(Journal.status, func.count().label("count")).group_by(
                        Journal.status
                    )
                )
            )
    ...
    async def count_search_title_or_abstract(self, keyword: str) -> int:
        ...
        return await self.count(
            (Journal.title.like(like_kw) | Journal.abstract.like(like_kw))
            & (Journal.status != "deleted")
        )

    async def list_search_title_or_abstract_page(
    ...
    async def list_search_author_page(
        self, keyword: str, page: int, page_size: int
    ) -> list[Dict[str, Any]]:
        """按作者关键词搜索分页"""
        offset = (page - 1) * page_size
    ...
    async def list_search_subject_page(
        self, subject: str, page: int, page_size: int
    ) -> list[Dict[str, Any]]:
        """按学科搜索分页"""
        offset = (page - 1) * page_size
        rows = (

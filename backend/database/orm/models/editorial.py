...
class EditorialBoard(Base):
    ...
    board_id: Mapped[int] = mapped_column(
        Integer, primary_key=True, comment="编委 ID（自增）"
    )
    user_uid: Mapped[int] = mapped_column(
        Integer, ForeignKey("users.uid"), unique=True, nullable=False, comment="用户 ID"
    )

    # 职位信息
    position: Mapped[str] = mapped_column(
        Text, nullable=False, comment="职位：editor/associate_editor/ea_ae"
    )
    title: Mapped[str | None] = mapped_column(Text, nullable=True, comment="职称/头衔")

    # 领域信息
    research_areas: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="研究领域（逗号分隔）"
    )

    # 任命信息
    appointed_at: Mapped[datetime] = mapped_column(
        DateTime, nullable=False, comment="任命时间"
    )
    appointed_by_uid: Mapped[int] = mapped_column(
        Integer, ForeignKey("users.uid"), nullable=False, comment="任命者用户 ID"
    )

    # 状态信息
    is_active: Mapped[bool] = mapped_column(
        default=True, nullable=False, server_default="true", comment="是否在职"
    )

    # 关联关系
    appointer: Mapped[User] = relationship(
        foreign_keys=[appointed_by_uid], lazy="joined"
    )
...
class DecisionRecord(Base):
    ...
    decision_id: Mapped[int] = mapped_column(
        BigInteger, primary_key=True, comment="决策 ID（自增）"
    )
    ...
    decision_type: Mapped[str] = mapped_column(
        Text, nullable=False, comment="决策类型：accept/reject/revision/transfer"
    )

    # 决策内容
    decision_title: Mapped[str] = mapped_column(
        Text, nullable=False, comment="决策标题"
    )
    decision_comments: Mapped[str] = mapped_column(
        Text, nullable=False, comment="决策意见"
    )
    recommendations: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="建议"
    )

    # 决策者信息
    decided_by_uid: Mapped[int] = mapped_column(
        Integer, ForeignKey("users.uid"), nullable=False, comment="决策者用户 ID"
    )
    decided_at: Mapped[datetime] = mapped_column(
        DateTime, nullable=False, comment="决策时间"
    )

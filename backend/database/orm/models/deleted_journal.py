...
class DeletedJournal(Base):
    ...
    jid: Mapped[int] = mapped_column(
        Integer, primary_key=True, comment="记录ID（自增）"
    )
    original_jid: Mapped[int] = mapped_column(
        BigInteger, nullable=False, comment="原文献ID（journals.jid）"
    )
    ...
    delete_time: Mapped[datetime] = mapped_column(
        DateTime, nullable=False, comment="删除时间"
    )

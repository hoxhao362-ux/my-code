...
class InvitationCode(Base):
    ...
    code_id: Mapped[int] = mapped_column(
        Integer, primary_key=True, comment="主键ID（自增）"
    )
    code: Mapped[str] = mapped_column(
        Text, unique=True, nullable=False, comment="邀请码（唯一）"
    )
    ...
    max_uses: Mapped[int] = mapped_column(
        Integer, nullable=False, server_default="1", comment="最大可使用次数"
    )
    used_count: Mapped[int] = mapped_column(
        Integer, nullable=False, server_default="0", comment="已使用次数"
    )

    description: Mapped[str | None] = mapped_column(Text, nullable=True, comment="描述")
    created_by: Mapped[str] = mapped_column(
        Text, nullable=False, comment="创建人用户名"
    )
    created_by_uid: Mapped[int] = mapped_column(
        Integer, ForeignKey("users.uid"), nullable=False, comment="创建人用户ID"
    )
...
class InvitationCodeUsage(Base):
    ...
    usage_id: Mapped[int] = mapped_column(
        Integer, primary_key=True, comment="使用记录ID（自增）"
    )
    code: Mapped[str] = mapped_column(Text, nullable=False, comment="邀请码")
    used_by_uid: Mapped[int] = mapped_column(
        Integer, ForeignKey("users.uid"), nullable=False, comment="使用人用户ID"
    )
    used_by_username: Mapped[str] = mapped_column(
        Text, nullable=False, comment="使用人用户名"
    )
    use_time: Mapped[datetime] = mapped_column(
        DateTime, nullable=False, comment="使用时间"
    )

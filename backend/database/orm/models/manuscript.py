...
class Manuscript(Base):
    ...
    manuscript_id: Mapped[int] = mapped_column(BigInteger, primary_key=True, comment="稿件 ID（自增，BIGSERIAL）")
    author_uid: Mapped[int] = mapped_column(Integer, ForeignKey("users.uid"), nullable=False, comment="投稿作者用户 ID")
    ...
    # 流转信息
    stage: Mapped[str] = mapped_column(Text, nullable=False, server_default="initial_review", comment="流转阶段：initial_review/peer_review/final_decision")
    status: Mapped[str] = mapped_column(Text, nullable=False, server_default="pending_initial_review", comment="全局状态")
    ...
    file_bucket: Mapped[str] = mapped_column(Text, nullable=False, comment="文件存储桶路径")
    ...
    files: Mapped[list["ManuscriptFile"]] = relationship(back_populates="manuscript", lazy="select")
...
class ManuscriptVersion(Base):
    ...
    version_id: Mapped[int] = mapped_column(BigInteger, primary_key=True, comment="版本 ID（自增）")
    manuscript_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("manuscripts.manuscript_id"), nullable=False, comment="稿件 ID")
    version_number: Mapped[int] = mapped_column(Integer, nullable=False, comment="版本号（从 1 开始）")
    ...
    # 关联关系
    manuscript: Mapped["Manuscript"] = relationship(back_populates="versions", lazy="joined")
...
class ManuscriptParticipant(Base):
    ...
    participant_id: Mapped[int] = mapped_column(Integer, primary_key=True, comment="参与者 ID（自增）")
    manuscript_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("manuscripts.manuscript_id"), nullable=False, comment="稿件 ID")
    user_uid: Mapped[int] = mapped_column(Integer, ForeignKey("users.uid"), nullable=False, comment="用户 ID")
...
class ManuscriptFile(Base):
    ...
    file_id: Mapped[int] = mapped_column(Integer, primary_key=True, comment="附件 ID（自增）")
    manuscript_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("manuscripts.manuscript_id"), nullable=False, comment="稿件 ID")
    
    # 文件信息
    file_hash: Mapped[str] = mapped_column(Text, nullable=False, comment="文件哈希")
    file_bucket: Mapped[str] = mapped_column(Text, nullable=False, comment="文件存储桶路径")
    original_name: Mapped[str] = mapped_column(Text, nullable=False, comment="原始文件名")
    file_size: Mapped[int] = mapped_column(BigInteger, nullable=False, comment="文件大小（字节）")
    content_type: Mapped[str] = mapped_column(Text, nullable=False, comment="文件 MIME 类型")
    ...
    # 关联关系
    manuscript: Mapped["Manuscript"] = relationship(back_populates="files", lazy="joined")

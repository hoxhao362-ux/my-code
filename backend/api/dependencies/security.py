...
async def get_token(
    authorization: Optional[str] = Header(
        None, description="认证头 Authorization: Bearer <token>"
    ),
) -> str:
    """
    获取认证令牌
...
async def get_reviewer_user(
    ...
    if not UserRole.has_permission(current_user["role"], UserRole.REVIEWER.value):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="需要审稿人权限"
        )
    return current_user
...
async def get_editor_user(
    ...
    if not UserRole.has_permission(current_user["role"], UserRole.EDITOR.value):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="需要编辑权限"
        )
    return current_user


async def get_associate_editor_user(
    current_user: Dict[str, Any] = Depends(get_current_active_user),
) -> Dict[str, Any]:
    """
    获取副编辑用户
    ...
    """
    if not UserRole.has_permission(
        current_user["role"], UserRole.ASSOCIATE_EDITOR.value
    ):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="需要副编辑权限"
        )
    return current_user


async def get_ea_ae_user(
    current_user: Dict[str, Any] = Depends(get_current_active_user),
) -> Dict[str, Any]:
    """
    获取编辑助理用户
    ...
    if not UserRole.has_permission(current_user["role"], UserRole.EA_AE.value):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="需要编辑助理权限"
        )
    return current_user
...
async def get_author_user(
    ...
    if not UserRole.has_permission(current_user["role"], UserRole.AUTHOR.value):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="需要作者权限"
        )
    return current_user

...
from database.orm.models.manuscript import (
    Manuscript, ManuscriptVersion, ManuscriptParticipant, ManuscriptFile
)
...
async def update_user_role(
    ...
    if not UserRole.is_valid(role):
        raise HTTPException(status_code=400, detail=f"角色无效，允许的角色: {UserRole.get_all_values()}")
...
async def delete_user(
    ...
    global_logger.info("AdminUsers", f"管理员 {current_user['username']} 软删除了用户 {uid}")
...
async def permanent_delete_user(
    ...
    """
    if not confirm:
        raise HTTPException(status_code=400, detail="请确认永久删除操作（confirm=true）")
    ...
    async with transactional(session):
        ...
        if user.role in [UserRole.REVIEWER.value, UserRole.ASSOCIATE_EDITOR.value, UserRole.EDITOR.value]:
        ...
        # 2.1 删除用户作为参与者关联的数据
        await session.execute(delete(ManuscriptParticipant).where(ManuscriptParticipant.user_uid == uid))
        await session.execute(delete(ManuscriptParticipant).where(ManuscriptParticipant.assigned_by_uid == uid))

        # 2.2 删除用户上传的稿件文件
        await session.execute(delete(ManuscriptFile).where(ManuscriptFile.uploaded_by_uid == uid))

        # 2.3 删除用户提交的稿件版本
        await session.execute(delete(ManuscriptVersion).where(ManuscriptVersion.submitted_by_uid == uid))
        ...
        if manuscript_ids:
            ...
            await session.execute(
                delete(ManuscriptFile).where(ManuscriptFile.manuscript_id.in_(manuscript_ids))
            )
            await session.execute(
                delete(ManuscriptParticipant).where(ManuscriptParticipant.manuscript_id.in_(manuscript_ids))
            )
            await session.execute(
                delete(ManuscriptVersion).where(ManuscriptVersion.manuscript_id.in_(manuscript_ids))
            )
            # 删除审稿意见
            await session.execute(
                delete(ReviewOpinion).where(ReviewOpinion.manuscript_id.in_(manuscript_ids))
            )
            # 删除决策记录
            await session.execute(
                delete(DecisionRecord).where(DecisionRecord.manuscript_id.in_(manuscript_ids))
            )
            # 最后删除稿件
            await session.execute(delete(Manuscript).where(Manuscript.manuscript_id.in_(manuscript_ids)))

        # 3. 删除审稿意见（用户作为审稿人）
        await session.execute(delete(ReviewOpinion).where(ReviewOpinion.reviewer_uid == uid))

        # 4. 删除决策记录（用户作为决策者）
        await session.execute(delete(DecisionRecord).where(DecisionRecord.decided_by_uid == uid))

        # 5. 删除编委会成员记录
        await session.execute(delete(EditorialBoard).where(EditorialBoard.user_uid == uid))
        await session.execute(delete(EditorialBoard).where(EditorialBoard.appointed_by_uid == uid))

        # 6. 删除邀请码相关记录
        await session.execute(delete(InvitationCode).where(InvitationCode.created_by_uid == uid))
        await session.execute(delete(InvitationCodeUsage).where(InvitationCodeUsage.used_by_uid == uid))
    ...
    global_logger.warning("AdminUsers", f"管理员 {current_user['username']} 永久删除了用户 {uid}")

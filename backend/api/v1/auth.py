...
from model.user import ChangePasswordRequest, LoginRequest, RegisterRequest
...
async def register(
    ...
    if not allowed:
        ...
        global_logger.warning(
            "Auth",
            f"注册频率超限 - IP: {client_ip}, 尝试次数：{attempts}/{cfg['max_attempts']}",
        )
        raise HTTPException(
            status_code=429,
...
>>>>>>> e47b4028170e280d7071481fe2e065479b0866ea
    if request.invite_code:
        ...
        validation_result = await invitation_service.validate_invitation_code(
            request.invite_code, session=session
        )
        if not validation_result["valid"]:
            global_logger.warning(
                "Auth",
                f"邀请码无效 - code: {request.invite_code}, reason: {validation_result['message']}",
            )
            raise HTTPException(
                status_code=400, detail=f"邀请码无效：{validation_result['message']}"
            )
        user_role = validation_result["role"]
        global_logger.info(
            "Auth",
            f"使用邀请码升级角色 - username: {request.username}, role: {user_role}",
        )
...
>>>>>>> e47b4028170e280d7071481fe2e065479b0866ea
        ...
        global_logger.info(
            "Auth",
            f"用户注册成功 - uid: {new_user.uid}, username: {new_user.username}, role: {user_role}",
        )
        ...
        global_logger.exception(
            "Auth", f"用户注册失败 - username: {request.username}, error: {e}"
        )
...
async def login(
    ...
    if not allowed:
        ...
        global_logger.warning(
            "Auth",
            f"登录频率超限 - IP: {client_ip}, 尝试次数：{attempts}/{cfg['max_attempts']}",
        )
        raise HTTPException(
            status_code=429,
    ...
    global_logger.info(
        "Auth",
        f"用户登录成功 - uid: {user.uid}, username: {user.username}, role: {user.role}",
    )
...
async def change_password(
    ...
    global_logger.info(
        "Auth", f"密码修改成功，已强制下线 - uid: {uid}, username: {username}"
    )

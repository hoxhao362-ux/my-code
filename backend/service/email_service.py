...
class EmailService(BaseManagedService):
    ...
    def _load_config(self):
        ...
        if sender_config:
            ...
            self._username = sender_config.get(
                "sender_email", sender_config.get("username", "")
            )
            self._password = sender_config.get(
                "sender_auth_code", sender_config.get("password", "")
            )
            ...
            email_config = (
                global_root.get("email", {}) if isinstance(global_root, dict) else {}
            )
            if email_config:
                self._smtp_server = email_config.get("smtp_server", "")
                self._smtp_port = self._parse_smtp_port(
    ...
    async def start(self):
        ...
        if not self._smtp_server or not self._username:
            ...
            global_logger.info(
                "Email", f"邮件服务已就绪: {self._smtp_server}:{self._smtp_port}"
            )
    ...
    async def send_email(
        ...
        try:
            ...
            global_logger.info(
                "Email", f"邮件发送成功: to={to_email}, subject={subject}"
            )
            return True
    ...
    async def send_email_with_attachments(
        ...
        try:
            ...
            global_logger.info(
                "Email", f"带附件邮件发送成功: to={to_email}, subject={subject}"
            )
            return True
    ...
    async def send_batch_emails(
        ...
        return {"success": success, "failed": failed, "failed_emails": failed_emails}
    ...
    async def send_verification_email(
        self, to_email: str, code_type: str = "verify_email", code: Optional[str] = None
        ...
        template = self._template_cache.get(template_key) or self._lookup_template_dict(
            template_key
        )
        ...
        try:
            content = content_template.format(
                verification_code, self._code_expire_minutes
            )
        except Exception:
            # 如果模板只期望一个参数
        ...
        success = await self.send_email(
            to_email, subject, content, content_type="plain"
        )

        if success:
            global_logger.info(
                "Email", f"验证码邮件发送成功: to={to_email}, type={code_type}"
            )
    ...
    async def send_reviewer_invitation(
        ...
        ) or self._lookup_template_dict("email.invite_reviewer")

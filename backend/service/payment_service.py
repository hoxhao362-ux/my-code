...
try:
    ...
    global_logger.warning(
        "Payment",
        "未检测到 alipay-sdk-python 库，支付宝功能将不可用。请运行 pip install alipay-sdk-python",
    )
...
class BasePaymentProvider(abc.ABC):
    ...
    @abc.abstractmethod
    def create_order(
        self, order_id: str, amount: float, subject: str, **kwargs
    ) -> PaymentResult:
        """创建支付订单"""
        pass
    ...
    @abc.abstractmethod
    def query_order(
        self, order_id: str, transaction_id: Optional[str] = None
    ) -> PaymentResult:
        """查询订单状态"""
        pass
...
class MockPaymentProvider(BasePaymentProvider):
    ...
    def create_order(
        ...
        self.logger.info(
            "Payment", f"[Mock] 创建订单: {order_id}, 金额: {amount}, 标题: {subject}"
        )
    ...
    def refund(
        ...
        self.logger.info(
            "Payment", f"[Mock] 订单退款: {order_id}, 金额: {refund_amount}"
        )
        return PaymentResult(
            success=True,
            message="Mock退款成功",
...
class AlipayProvider(BasePaymentProvider):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        alipay_public_key_string = self._read_key_file(
            config.get("alipay_public_key_path")
        )
        ...
        self.logger.info(
            "Payment",
            f"初始化支付宝提供商 (AppID: {self.app_id}, Sandbox: {self.debug})",
        )
    ...
    def verify_notify(self, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if is_valid:
                self.logger.info(
                    "Payment", f"支付宝回调验签成功: {data.get('out_trade_no')}"
                )
            else:
                self.logger.error(
                    "Payment", f"支付宝回调验签失败: {data.get('out_trade_no')}"
                )
...
class PaypalProvider(BasePaymentProvider):
    ...
    def refund(
        ...
        self.logger.info(
            "Payment", f"[PayPal] 订单退款: {order_id}, 金额: {refund_amount}"
        )
        return PaymentResult(
            success=True,
            message="PayPal退款成功",
    ...
    def initialize(self):
        ...
        try:
            if provider_type == "alipay":
                ...
                global_logger.warning(
                    "Payment", f"未知的支付提供商: {provider_type}，使用 Mock"
                )
                self.provider = MockPaymentProvider(common_config)

    ...
    async def generate_captcha(self) -> Tuple[str, str]:
        ...
        try:
            ...
            if not redis_service.client:
                global_logger.error(
                    "Captcha", "生成验证码时发现 Redis 未连接，无法缓存验证码"
                )
                raise Exception("Redis 服务不可用")

            await redis_service.client.setex(
                name=redis_key, time=self.expire_seconds, value=text.lower()
            )

            global_logger.debug(
                "Captcha",
                f"图形验证码已生成并存入缓存: req_id={req_id}, expires={self.expire_seconds}s",
            )
    ...
    async def verify_captcha(self, req_id: str, code: str) -> bool:
        ...
        try:
            ...
            if real_code is not None:
                ...
                global_logger.debug(
                    "Captcha", f"验证码验证失败：请求ID不存在或已过期 req_id={req_id}"
                )
                return False
            ...
            if is_valid:
                ...
                global_logger.debug(
                    "Captcha",
                    f"图形验证码验证失败：输入不匹配 req_id={req_id}, input={code}, real={real_code}",
                )

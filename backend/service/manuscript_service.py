...
from core.enums import (DecisionType, ManuscriptStatus, ReviewStage, UserRole,
                        WorkflowAction)
...
TRANSITION_MAP: dict[tuple[str, str, Optional[str]], str] = {
    ...
    (
        ManuscriptStatus.UNDER_INITIAL_REVIEW.value,
        WorkflowAction.DECIDE.value,
        DecisionType.REVISION.value,
    ): ManuscriptStatus.INITIAL_REVIEW_REVISION.value,
    (
        ManuscriptStatus.UNDER_INITIAL_REVIEW.value,
        WorkflowAction.DECIDE.value,
        DecisionType.REJECT.value,
    ): ManuscriptStatus.INITIAL_REVIEW_REJECTED.value,
    (
        ManuscriptStatus.UNDER_INITIAL_REVIEW.value,
        WorkflowAction.DECIDE.value,
        DecisionType.TRANSFER.value,
    ...
    (
        ...
        WorkflowAction.DECIDE.value,
        DecisionType.ACCEPT.value,
    ): ManuscriptStatus.FINAL_DECISION_ACCEPTED.value,
    (
        ManuscriptStatus.UNDER_FINAL_DECISION.value,
        WorkflowAction.DECIDE.value,
        DecisionType.REVISION.value,
    ...
    (
        ...
        None,
    ): ManuscriptStatus.PENDING_ACCEPTANCE_CONFIRMATION.value,
    # 待录用确认 → 版权协议
    (
        ManuscriptStatus.PENDING_ACCEPTANCE_CONFIRMATION.value,
        WorkflowAction.APPROVE.value,
        None,
    ): ManuscriptStatus.PENDING_COPYRIGHT.value,
    # 待版权协议 → 校样确认
    (
        ManuscriptStatus.PENDING_COPYRIGHT.value,
        WorkflowAction.APPROVE.value,
        None,
    ): ManuscriptStatus.PENDING_PROOF.value,
    # 待校样确认 → 待出版
    (
        ManuscriptStatus.PENDING_PROOF.value,
        WorkflowAction.APPROVE.value,
        None,
    ): ManuscriptStatus.PENDING_PUBLICATION.value,
    # 待出版 → 已出版
    (
        ManuscriptStatus.PENDING_PUBLICATION.value,
...
class ManuscriptWorkflowService:
    ...
    @staticmethod
    def get_next_status(
        current_status: str, action: str, decision_type: Optional[str] = None
    ) -> Optional[str]:
        """获取下一个状态
    ...
    async def execute_action(
        ...
        query = select(Manuscript).where(
            Manuscript.manuscript_id == manuscript_id, Manuscript.is_deleted == False
        )
        ...
        if action == WorkflowAction.DECIDE.value and not decision_type:
            raise ValueError(
                "决策动作需要指定决策类型 (accept/reject/revision/transfer)"
            )
